...
async def usuario_lista(id_usuario: int):
    ...
    if perfil.cod_perfil == Const.K_ADMINISTRADOR_CARRERA.value:
        query = " \
            ...
            where u.cod_carrera = (select us.cod_carrera \
                                from usuario us \
                                where us.id_usuario = %s) and segundo_apellido!='' and \
    ...
    if perfil.cod_perfil == Const.K_ADMINISTRADOR_TI.value:
        ...
        select u.id_usuario as id_usuario, \
            ...
            from usuario u \
            join perfil p on u.cod_perfil = p.cod_perfil \
            left join carrera c on u.cod_carrera = c.cod_carrera where segundo_apellido!='' \
            order by u.cod_carrera asc, \
                u.cod_perfil asc, \
                u.primer_apellido asc, \
                u.segundo_apellido asc, \
...
# mio
@router.delete("/api/usuario/deshabilitar/{id_usuario_eliminar}/{id_usuario}", response_model=dict, summary="Deshabilita un usuario", tags=["Usuarios"])
async def usuario_deshabilitar(id_usuario_eliminar: int, id_usuario: int):
